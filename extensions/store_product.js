...
//				app.u.dump(" -> ID before any manipulation: "+$tag.attr('id'));
				...
				if(app.ext.store_product.u.productIsPurchaseable(pid))	{
					...
					$tag.hide().addClass('displayNone').before("<span class='notAvailableForPurchase'>Currently out of stock.<\/span>"); //hide button, item is not purchaseable.
					}
...
*/
			showPicsInModal : function(P)	{
				if(P.pid)	{
					...
					P.width = P.width ? P.width : 680;
					P.height = P.height ? P.height :555;
...
//parent may not exist. empty if it does, otherwise create it.
					...
					if(P.templateID)	{
						...
						$parent.append(app.u.makeImage({"class":"imageViewerSoloImage","w":"640","h":"480","bg":"ffffff","name":app.data['appProductGet|'+P.pid]['%attribs'][imageAttr],"tag":1}));
						}	
