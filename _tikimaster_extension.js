...
/*				app.u.dump('BEGIN app.ext.tikimaster.callbacks.populateHotItemsList.onSuccess');
				app.u.dump(tagObj);
			...
			onError : function(){
				app.u.dump('BEGIN app.ext.tikimaster.callbacks.populateHotItemsList.onError');
				}
			}
		}, //callbacks
...
//these are going the way of the do do, in favor of app events. new extensions should have few (if any) actions.
		a : {
			...
			showHomepageSlideshow : function(){
				if(app.ext.tikimaster.vars.homepageBanners){
					...
					setTimeout(function(){app.ext.tikimaster.u.showHomepageSlideshow();}, 250);
					}
				},
			...
			addRMAItem : function(){
				...
				$rmaItem.append($('<button onClick="app.ext.tikimaster.a.removeRMAItem($(this).parent()); return false;" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only"><span class="ui-button-icon-primary ui-icon ui-icon-closethick"></span></button>'));
			...
			showDropDownClick : function($tag){
				...
				if(this.showDropDown($tag)){
					...
					setTimeout(function(){$('body').click(function(){
						app.ext.tikimaster.a.hideDropDownClick($tag);
						});}, 500);
					}
				},
			...
			hideDropDownClick : function($tag){
				...
				if(this.hideDropDown($tag)){
					$tag.click(function(){app.ext.tikimaster.a.showDropDownClick($(this));});
					$('body').unbind('click');
					}
				},
...
//when adding an event, be sure to do off('click.appEventName') and then on('click.appEventName') to ensure the same event is not double-added if app events were to get run again over the same template.
		...
		vars : {
			...
			catTemplates : {
				...
				'.in_the_news.tikimaster_expansion_press_release' : 'categoryTemplateHTML',
				'.in_the_news.tikimaster_shows_northwestern_support_at_wrigley_field' : 'categoryTemplateHTML',
				'.in_the_news.tikimaster_webcam' : 'categoryTemplateHTML',
				'.in_the_news.tikimastercom_puts_the_eye_on_wrigleyville_with_new_bar_web_cam' : 'categoryTemplateHTML',
				'.in_the_news.tikimastercom_unveils_new_rewards_program' : 'categoryTemplateHTML',
				...
				'.in_the_news.sports_world___s_online_store_tikimastercom_changes_name' : 'categoryTemplateHTML',
